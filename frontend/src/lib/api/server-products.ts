...
export async function fetchProductsServer(locale?: string): Promise<Product[]> {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/products?limit=50${query}`, { next: { revalidate: 60 } });
    if (!res.ok) return [];
...
export async function fetchProductBySlugServer(slug: string, locale?: string): Promise<Product | null> {
  try {
    const query = locale ? `?locale=${encodeURIComponent(locale)}` : "";
    const res = await fetch(`${API_BASE_URL}/products/${slug}${query}`, { next: { revalidate: 60 } });
