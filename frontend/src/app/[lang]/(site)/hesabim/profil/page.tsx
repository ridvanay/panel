...
/**
 * `/hesabim/profil` — §10.21 §8.3 tablosu: ADMIN/MANAGER/EDITOR/CUSTOMER/USER'ın HEPSİ
 * erişebilir. Profil (ad/avatar), şifre ve 2FA + oturumlar. Auth guard'ı artık `hesabim/layout.tsx`
 * (`HesabimShell`) içindedir — bu bileşen yalnızca `status === "authenticated"` iken mount edilir
 * (bkz. `.claude/architect-scope-customer-portal.md` §7.2).
 */
export default function AccountProfilePage() {
  ...
  // `HesabimShell` bu sayfayı yalnızca `status === "authenticated"` iken mount eder — bu
  // savunma amaçlı bir erken çıkıştır (TS null daraltması), pratikte tetiklenmez.
  if (!user) return null;
