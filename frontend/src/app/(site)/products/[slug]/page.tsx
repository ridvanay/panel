...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const [product, settings] = await Promise.all([fetchProductBySlugServer(slug), fetchSiteSettingsServer()]);
  if (!product) return {};
  ...
  return buildContentMetadata(
    ...
    {
      fallbackCanonicalUrl: `${SITE_URL}/products/${slug}`,
...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  return (
    <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      ...
      <h1 className="mt-1 text-3xl font-semibold text-foreground">{product.title}</h1>
