...
import { AlertCircle, Briefcase, Download, Search, Tag } from "lucide-react";
...
function matchesPortfolioItem(item: PortfolioItem, query: string): boolean {
  return item.title.toLowerCase().includes(query) || (item.clientName ?? "").toLowerCase().includes(query);
}

/** Portföy kategorisi, `ContentListEntity`'nin ORTAK alan setinde yok — bkz. `csv-columns.ts`. */
const portfolioCsvColumns = buildContentCsvColumns<PortfolioItem>((item) => item.category?.name ?? "—");

export default function AdminPortfolioListPage() {
  const { user } = useAuth();
  const isAdmin = user?.role === "ADMIN";

  const list = useContentList<PortfolioItem>({
    fetchList: portfolioApi.listPortfolioItems,
    updateItem: (id, input) => portfolioApi.updatePortfolioItem(id, input),
    trashItem: portfolioApi.deletePortfolioItem,
    restoreItem: portfolioApi.restorePortfolioItem,
    permanentDeleteItem: portfolioApi.permanentDeletePortfolioItem,
    bulkAction: portfolioApi.bulkPortfolioItemsAction,
    matches: matchesPortfolioItem,
    nounSingular: "Portföy öğesi",
  });

  function handleExport() {
    if (list.tabItems.length === 0) return;
    exportToCsv("portfoy.csv", list.tabItems, portfolioCsvColumns);
  }

  function handleBulkExport() {
    const selectedItems = list.tabItems.filter((item) => list.selectedIds.has(item.id));
    if (selectedItems.length === 0) return;
    exportToCsv("secili-portfoy.csv", selectedItems, portfolioCsvColumns);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeading
        icon={Briefcase}
        title="Portföy"
        description="Portföy projelerinin listesi."
        actions={
          <div className="flex items-center gap-2">
            ...
            <LinkButton href="/admin/portfolio/categories" variant="outline">
              <Tag className="h-4 w-4" />
              Kategoriler
            </LinkButton>
            <LinkButton href="/admin/portfolio/new">Yeni Proje</LinkButton>
          </div>
        }
      />
      ...
      {list.loading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <EmptyState
            icon={Briefcase}
            title="Henüz portföy öğesi yok"
            description="İlk projenizi oluşturarak başlayın."
            action={<LinkButton href="/admin/portfolio/new">Yeni Proje</LinkButton>}
          />
        </motion.div>
      ) : (
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <InputGroup className="w-full sm:max-w-xs border-2 border-border bg-muted">
              ...
              <InputGroupInput
                placeholder="Başlığa veya müşteri adına göre ara..."
                aria-label="Başlığa veya müşteri adına göre ara"
                value={list.search}
          ...
          {list.filteredCount === 0 ? (
            <EmptyState icon={Search} title="Sonuç bulunamadı" description="Arama kriterlerinize uyan bir proje yok." />
          ) : (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <ContentListTable
                ...
                viewHref={(item) => `/portfolio/${item.slug}`}
                extraColumns={[
                  {
                    key: "category",
                    header: "Kategori",
      ...
      <ConfirmDialog
        ...
        title="Portföy öğesini kalıcı sil"
        description={
          list.pendingPermanentDelete
            ? `"${list.pendingPermanentDelete.title}" öğesini kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`
            : undefined
        }
      ...
      <ConfirmDialog
        ...
        title="Portföy öğelerini kalıcı sil"
        description={`${list.selectedIds.size} öğeyi kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`}
        confirmText="Kalıcı Sil"
        destructive
