...
import type { Slide, Slider, SliderLayer, SliderLayerType } from "@/lib/sliders/types";
...
type InspectorTabValue = "slide" | "layer" | "animation" | "slider";
...
export function HeroStudioInspector({
  ...
  const [tab, setTab] = useState<InspectorTabValue>("slide");

  return (
    <aside className="w-[380px] shrink-0 overflow-y-auto border-l border-border bg-surface p-4">
      <Tabs value={tab} onValueChange={(v) => setTab(v as InspectorTabValue)}>
        ...
        <TabsContent value="layer" className="pt-4">
          <LayerInspectorTab layer={layer} device={device} onUpdateLayer={onUpdateLayer} onDeleteLayer={onDeleteLayer} onAddLayer={onAddLayer} />
        </TabsContent>
