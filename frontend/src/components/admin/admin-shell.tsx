...
/**
 * `/admin/**` kabuğu (sidebar/topbar/breadcrumb, command palette, klavye kısayolları, auth
 * yönlendirmesi). Client Component olmak ZORUNDA (hooks, framer-motion) — bu yüzden
 * `metadata`/`generateMetadata` EXPORT EDEMEZ (Next.js kısıtı). Dinamik `<title>` üretimi
 * `frontend/src/app/admin/layout.tsx`teki SERVER `AdminLayout`e taşındı, o bu bileşeni
 * `children` olarak sarmalar (görev: 2026-09-16, "SaaS Platform" jenerik başlığının admin
 * panelinden kaldırılması).
 */
