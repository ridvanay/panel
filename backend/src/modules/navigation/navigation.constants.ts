 ...
 *
 * Bu değerler `frontend/src/lib/navigation-constants.ts` içinde AYNALANIR (proje bir npm
 * workspace monorepo'su olmadığı için gerçek bir `shared/` paketi orantısız — bkz.
 * ARCHITECTURE.md §10.10.1 "Sabitin yeri"). qa-agent iki dosyanın değer eşitliğini test
 * eder. Derinliği değiştirmek istenirse SADECE bu iki dosya + kontrat güncellenir, kod
 * mantığı derinlikten bağımsızdır.
 */
